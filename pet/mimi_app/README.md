...
- **好感度**：Mimi 使用独立的 `0..100` 关系值（初始 50），摸头、击掌、成功投喂和桌宠模式输入框发送的正向聊天会在成功后小幅增加；聊天有 90 秒冷却、每日 3 点额度和同文指纹去重，并通过 happy 表情与阶段气泡反馈。Agent 的 assistant 回复、工具调用和问题卡回答不单独加分。工作模式（`link`）中的 DSH 任务、工具调用、失败、断线和模式切换**不会增加或降低好感度**。状态默认保存到 `%APPDATA%\MimiDesktopPet\state.json`，可用 `MIMI_STATE_PATH` 覆盖；饱腹度仍是独立的 `0..1` 内存状态。
- DeepSeek“鲸鱼娘”采用社区网络梗的安全角色转译，不声称是已证实的官方隐藏人格；危险操作确认和其他安全边界不受亲密度影响。
