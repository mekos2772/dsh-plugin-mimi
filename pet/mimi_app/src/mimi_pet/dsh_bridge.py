...
class DshEventThread:
    ...
    def __init__(
        ...
        # Fired once when the reader stops for a reason retrying cannot fix.
        # Unlike on_disconnect this must explain itself: the thread used to die
        # before its first log line, so a missing optional dependency looked
    ...
    def _fatal(self, reason: str) -> None:
        """Report a permanent failure once; the reader thread then exits."""
        if self._fatal_reported:
            return
        self._fatal_reported = True
        dbg(f"dsh event reader unavailable: {reason}")
    ...
    def _run(self) -> None:
        try:
            ...
            # Retrying cannot help — the package will not appear mid-run. This
            # used to raise out of the thread before its first statement, so
            # the pet silently lost every streamed token, tool row and approval
            # card while looking merely "disconnected": an agent could block on
            # a question whose card was never delivered.
            self._fatal(
                "缺少 websocket-client 依赖，收不到 DSH 实时事件（"
