...
- 右键菜单对运行时长文本统一截断并保留完整 tooltip，增加弹出宽度上限，避免媒体标题、焦点状态或 DSH 错误把菜单撑满屏幕。
- 音乐栏改为默认关闭，并提供显示/隐藏切换与关闭按钮；媒体状态保持可读且不再强制占用桌面。
- 启动时检查运行素材，修复已安装包中损坏的 `sit_down` 第 46 帧 `0046.webp`。
- 增加菜单标签、媒体菜单形状和长运行时文本的回归测试；完整 Python 测试通过。
