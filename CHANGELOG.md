...
- 内置 Computer Use 升级到 `@milkuovo/dsh-computer-use@0.2.2`：取消信号改为随 `invokePowerShell` 的 options 传递（此前塞在参数对象里，取消实际不生效）；指针动画在 abort 时取消并收尾 overlay；每次执行前检查 `signal.throwIfAborted()`。同时修复窗口定位与 MCP 传输。
- 新增**更新公告**：更新后第一次打开会说明本版本改了什么，跨版本会聚合列出；完整列表进入 DSH 面板的「最近通知」，桌宠同时说一句。
- 新增**检查更新**：保留连接 DSH 后的自动检查，右键菜单加入手动入口，「已是最新 / 查不到」也有反馈。

## 0.7.4 — 2026-09-14

- DSH 陪伴面板界面验收完成：会话顶部 `Mimi` 入口、右侧「Mimi · 专注与提醒」面板、专注计时、勿扰、一次性提醒与通知历史。
- 头顶气泡按文字自适应：上限放宽到 340px / 6 行，`\n` 作为硬换行，溢出追加省略号，窄气泡在图层内居中。
- 右键菜单改在释放时打开并置顶；弹出层展开期间不再移动桌宠窗口或升降其它置顶窗口。
- 新增专注悬浮时钟：跟随桌宠的圆形表盘，显示倒计时与状态，可拖动固定、双击归还。
- 内置 Computer Use 为 `0.2.1`。
