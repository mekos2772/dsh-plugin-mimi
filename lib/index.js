...
export const Config = z.object({
  ...
  companionEnabled: z.boolean().default(true).description('在 DSH 内启用 Mimi 专注、勿扰与提醒（更改后重启插件）'),
  focusMinutes: z.number().min(1).max(240).step(1).default(25).description('默认专注时长，单位分钟（更改后重启插件）'),
...
export function apply(ctx, config) {
  const settingsScope = ctx.settings.register('mimi-pet', Config, { base: config ?? {}, applies: 'restart' });
  const connection = ctx.connection;
  const dshHost = resolveDshHost(ctx.webServer);
  const defaults = Config({});
  // The host and Python child use the same configuration for this lifetime.
  const activeConfig = { ...defaults, ...settingsScope.get() };
